...
def display_map (data, géo) :
    ...
    attach_tooltip(choropleth)
    choropleth.add_to(m)
    colors = create_colors(data, géo)
    colors.add_to(m)
    # create_indexes(géo, m)
...
def create_choropleth (data, géo) :
    return folium.Choropleth(
        ...
        name='choropleth',
        data=data,
        columns=['code_dep', 'TOTAL'],  # 'code' is the department code, 'TOTAL' is wine production
        key_on='feature.properties.code',  # Key for matching the data with the GeoJSON features
        fill_color='YlGn',
        fill_opacity=0.7,
        line_opacity=0.2,
...
def render_streamlit_map(map, key):
    ...
    if st.button("Click here to select all deparments"):
        st.session_state["dep"] = "00"
...
def attach_tooltip(choropleth):
    fields = (
        ["code", "nom"]
    )
    tooltip_style = "background-color: lightgreen;"
    choropleth.geojson.add_child(
        folium.features.GeoJsonTooltip(fields, labels=False, style=tooltip_style)
    )
...
def create_cognac_pie(data):
    ...
    fig.update_layout(title_text="Production de Cognac par département")
    return fig
...
def linear_chart(data):
    ...
    fig.update_layout(title_text="Productivité par département")
    # fig = go.Figure(go.Scatter(x=data['nombre de déclarations'], y=data['TOTAL'], mode='lines'))
    # fig.update_layout(title_text="Droite TOTAL par département")
    return fig
...
def top_n_dep_wine(n, wine_type, ordre, data):
    ...
    fig.update_layout(title_text="Les 5 départements produisant le plus")
    return fig
...
def dispay_choice():
    selected_columns = ["qte AOC blanc", "qte AOC rouge/rose", "qte VDQS blanc", "qte VDQS rouge/rose",
                    "qte VDP blanc", "qte VDP rouge/rose", "qte autre blanc",
                    "qte autre rouge/rose", "qte totale blanc", "qte totale rouge/rose"]
    ...
    ordre = st.radio("Afficher le 'top' ou le 'bottom'", ["top", "bottom"])
